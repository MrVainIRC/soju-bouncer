...
func (u *user) run() {
	...
	for e := range u.events {
		...
		case eventUpstreamConnected:
			...
			u.notifyBouncerNetworkState(ctx, uc.network.ID, irc.Tags{
				"state": "connected",
				"error": "",
			})
		...
		case eventUpstreamConnectionError:
			...
			net.lastError = e.err
			u.notifyBouncerNetworkState(ctx, net.ID, irc.Tags{
...
func (u *user) notifyBouncerNetworkState(ctx context.Context, netID int64, attrs irc.Tags) {
...
func (u *user) createNetwork(ctx context.Context, record *database.Network, enforceLimit bool) (*network, error) {
	...
	u.notifyBouncerNetworkState(ctx, network.ID, attrs)
...
func (u *user) updateNetwork(ctx context.Context, record *database.Network, enforceLimit bool) (*network, error) {
	...
	u.notifyBouncerNetworkState(ctx, updatedNetwork.ID, attrs)
