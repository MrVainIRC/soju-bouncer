...
func (uc *upstreamConn) sendNextPendingCommand(cmd string) {
	...
	ctx := downstreamLabelContextWith(context.TODO(), pendingCmd.downstreamID, &downstreamLabelContext{
		label: pendingCmd.downstreamLabel,
	})
	uc.SendMessageLabeled(ctx, pendingCmd.downstreamID, pendingCmd.msg)
	pendingCmd.sentAt = time.Now()
}

func (uc *upstreamConn) enqueueCommand(ctx context.Context, dc *downstreamConn, msg *irc.Message) {
	var label string
